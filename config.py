...
ANCHOR_SCALES = (2.22152954, 4.12315647, 7.21692515,
                 12.60263013, 22.7102731)  # (4, 8, 16, 32)
...
class ModelConfig(object):
    ...
    def __init__(self):
        ...
        # if self.model not in ('motifnet', 'linknet', 'stanford', 'arpn'):
    ...
    def setup_parser(self):
        ...
        parser.add_argument(
            '-coco', dest='coco', help='Use COCO (default to VG)', action='store_true')
        ...
        parser.add_argument('-b', dest='batch_size',
                            help='batch size per GPU', type=int, default=2)
        parser.add_argument('-val_size', dest='val_size',
                            help='val size to use (if 0 we wont use val)', type=int, default=5000)

        parser.add_argument(
            '-l2', dest='l2', help='weight decay', type=float, default=1e-5)
        parser.add_argument(
            '-clip', dest='clip', help='gradients will be clipped to have norm less than this', type=float, default=5.0)
        ...
        parser.add_argument('-model', dest='model', help='which model to use? (motifnet, stanford). If you want to use the baseline (NoContext) model, then pass in motifnet here, and nl_obj, nl_edge=0', type=str,
                            default='motifnet')
        parser.add_argument('-old_feats', dest='old_feats',
                            help='Use the original image features for the edges', action='store_true')
        ...
        parser.add_argument('-cache', dest='cache', help='where should we cache predictions', type=str,
                            default='')
        parser.add_argument('-gt_box', dest='gt_box',
                            help='use gt boxes during training', action='store_true')
        parser.add_argument(
            '-adam', dest='adam', help='use adam. Not recommended', action='store_true')
        parser.add_argument('-test', dest='test',
                            help='test set', action='store_true')
        ...
        parser.add_argument('-resnet', dest='use_resnet',
                            help='use resnet instead of VGG', action='store_true')
        parser.add_argument('-proposals', dest='use_proposals',
                            help='Use Xu et als proposals', action='store_true')
        parser.add_argument('-nl_obj', dest='nl_obj',
                            help='Num object layers', type=int, default=1)
        parser.add_argument('-nl_edge', dest='nl_edge',
                            help='Num edge layers', type=int, default=2)
        parser.add_argument('-hidden_dim', dest='hidden_dim',
                            help='Num edge layers', type=int, default=256)
        parser.add_argument('-pooling_dim', dest='pooling_dim',
                            help='Dimension of pooling', type=int, default=4096)
        parser.add_argument('-pass_in_obj_feats_to_decoder',
                            dest='pass_in_obj_feats_to_decoder', action='store_true')
        parser.add_argument('-pass_in_obj_feats_to_edge',
                            dest='pass_in_obj_feats_to_edge', action='store_true')
        ...
        parser.add_argument('-rel_dim', dest='rel_dim',
                            help='Dimension of Relation embedding', type=int, default=200)

        parser.add_argument('-lambda1', dest='lambda1', help='weight for neg reward loss', type=float, default=100)
        parser.add_argument('-lambda2', dest='lambda2', help='weight for ranking loss', type=float, default=1.0)

        parser.add_argument('-loss', dest='loss', help='loss for relation prediction', type=str, default='ce')

        parser.add_argument('-use_rank', dest='use_rank',  action='store_true')
        parser.add_argument('-m1', dest='m1', help='margin for max margin loss', type=float, default=0.0)
        parser.add_argument('-m2', dest='m2', help='margin for max margin loss', type=float, default=0.0)
        parser.add_argument('-gamma', dest='gamma', help='gamma for focal loss', type=float, default=2.0)

        parser.add_argument('-not_rdist', dest='not_rdist',  action='store_true')
        parser.add_argument('-use_score', dest='use_score',  action='store_true')
        parser.add_argument('-adapt_margin', dest='adapt_margin',  action='store_true')

        parser.add_argument('-early_stop', dest='early_stop',  action='store_true')

        parser.add_argument('-rloss', dest='rloss', help='loss for ranker', type=str, default='bce')

        parser.add_argument('-seed', dest='seed',
                            help='The answer to the ultimate question in the universe', type=int, default=42)
