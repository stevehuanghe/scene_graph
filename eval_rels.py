...
if conf.model == 'motifnet':
    ...
    from lib.rel_model_arpn import RelModelARPN as RelModel
else:
    raise ValueError()
...
def val_batch(batch_num, b, evaluator, thrs=(20, 50, 100), meta=None):
    det_res = detector[b]
    if conf.num_gpus == 1:
        det_res = [det_res]
...
epoch = conf.ckpt.split('/')[-1][6:].split('.')[0]
conf.cache =  '/'.join(conf.ckpt.split('/')[:-1]) + '/' + conf.mode + '-predictions-e'+ epoch + '.pkl'
filename = str(conf.cache).split('.')[0] + '.txt'
if conf.cache is not None and os.path.exists(conf.cache):
    ...
    evaluator[conf.mode].print_stats(filename=filename)
else:
    epoch = conf.ckpt.split('/')[-1][6:].split('.')[0]
    conf.cache =  '/'.join(conf.ckpt.split('/')[:-1]) + '/' + conf.mode + '-predictions-e'+ epoch + '.pkl'
    detector.eval()
    print(f'results will be saved at {conf.cache}')
    ...
    print(f'results saved at {conf.cache}')
