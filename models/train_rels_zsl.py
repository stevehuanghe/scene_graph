...
from dataloaders.pred_embed_loader import load_predicate_embeddings
from backup.visual_genome import VGDataLoader, VG
import numpy as np
...
from backup.config import ModelConfig, BOX_SCALE, IM_SCALE, GLOVE_VEC_FN
...
if conf.model == 'motifnet':
    ...
    from backup.rel_model_linknet_zsl import RelModelLinknet as RelModel
...
embedings = load_predicate_embeddings(GLOVE_VEC_FN, conf.rel_dim)
pred_embed = Variable(torch.Tensor(embedings), requires_grad=False).cuda()
...
detector = RelModel(classes=train.ind_to_classes, rel_classes=train.ind_to_predicates,
                    ...
                    rel_embed_dim=conf.rel_dim,
                    rel_embed=pred_embed
                    )
...
def train_epoch(epoch_num):
    ...
    for b, batch in enumerate(train_loader):
        tr.append(train_batch(batch, verbose=b %
                              (conf.print_interval*10) == 0))  # b == 0))
...
def train_batch(b, verbose=False):
    ...
    relations = result.rel_labels[:, -1].contiguous()
    gt_embed = pred_embed[relations.view(-1)]

    losses['mse_loss'] = F.mse_loss(result.rel_dists, gt_embed)
